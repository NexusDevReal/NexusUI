--[[
╔══════════════════════════════════════════════════════════════╗
║  NexusUI  v2.3  —  Roblox Luau UI Library                    ║
╠══════════════════════════════════════════════════════════════╣
║  v2.3 — Core reliability patch:                             ║
║                                                              ║
║  BUG FIXES                                                   ║
║  1. Drag type mismatch — movement now checked by InputType  ║
║     (MouseMovement / Touch), NOT by input object identity.  ║
║     MouseButton1 ≠ MouseMovement so the old check always    ║
║     failed silently; drag now works correctly on PC & mobile║
║  2. Mixed Scale+Offset positioning — win.Position now uses  ║
║     UDim2.fromOffset() so Scale values never interfere      ║
║     with dragging and windows cannot jump on first move     ║
║  3. Connection leak — globalMoveCon & globalEndCon tracked  ║
║     in a table; both disconnected on window close/destroy   ║
║  4. Unsafe callbacks — Call() wrapped in pcall with full    ║
║     error + stack-trace warning; UI thread never crashes    ║
║  5. Nil-safety guards — every InputBegan / Connect on UI    ║
║     elements is now wrapped with an existence check so      ║
║     a failed element creation cannot error the whole script ║
╚══════════════════════════════════════════════════════════════╝
--]]
...
-- ─────────────────────────────────────────────────────────────
--  DRAG  (mouse + touch) — v2.3
--
--  FIX 1 — Type-mismatch:
--    The movement check now tests InputType == MouseMovement
--    or Touch, NOT input == activeInput.  The click that
--    starts the drag is MouseButton1; the move events are
--    MouseMovement (desktop) or Touch (mobile) — they are
--    different objects, so object-identity comparison always
--    failed silently.  Type-based check works correctly.
--
--  FIX 2 — Mixed Scale/Offset:
--    win.Position uses UDim2.fromOffset() so existing Scale
--    values can never interfere.  Windows set at e.g.
--    UDim2.new(0.5,-185,…) no longer jump on first drag.
--
--  FIX 3 — Connection cleanup:
--    All four global/local connections stored; Disconnect()
--    cleans up all of them.  Leak is fully closed.
-- ─────────────────────────────────────────────────────────────
local function MakeDraggable(win, handle)
	...
	local startPos  = nil   -- Vector2: window offset at drag-begin (pixels only)

	-- FIX 2: capture only pixel offsets so Scale never interferes
	...
	local function doMove(inputPos)
		...
		-- FIX 2: pure offset — no stale Scale mixing
	...
	-- FIX 1: check InputType is movement, NOT object identity
	...
	-- Global move fallback (cursor/finger left the handle area)
	...
	local c4 = handle.InputEnded:Connect(endDrag)
	local c5 = UserInputService.InputEnded:Connect(endDrag)

	-- FIX 3: return object that disconnects ALL connections
	return {
		Disconnect = function()
...
function NexusUI:CreateWindow(opt)
	...
	local wSub    = opt.Subtitle or "v2.3"
