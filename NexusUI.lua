--[[
╔══════════════════════════════════════════════════════════════╗
║  NexusUI  v2.5  —  Roblox Luau UI Library                    ║
╠══════════════════════════════════════════════════════════════╣
║  v2.4 — Stability & performance patch (see git history)     ║
║                                                              ║
║  v2.5 — Bug-fix & polish release:                           ║
║                                                              ║
║  1. Frame zi/clip swap — 5 call-sites had the last two      ║
║     arguments reversed; overlay was ZIndex 1 (dropdowns     ║
║     hidden under content), body/win clipped descendants     ║
║     (dropdown lists cut off), body.ZIndex = true (type err) ║
║  2. MkIcon NoTint false-trap — classic Lua and/or ternary   ║
║     silently coerced NoTint=false to true; fixed with safe  ║
║     nil-check pattern                                       ║
║  3. TweenCache GC leak — table now uses weak keys so        ║
║     destroyed instances are not retained by the cache       ║
║  4. ColorPicker Set() ring — Set() cleared the active ring  ║
║     but never highlighted the matching swatch; added a      ║
║     swatchRings lookup table to restore the indicator       ║
║  5. Dropdown row leak — BuildRows used r.Parent=nil which   ║
║     left rows alive with all their signal connections;      ║
║     now uses r:Destroy() in both Dropdown & MultiDropdown   ║
║  6. Keybind Reg — AddKeybind never called Reg(), so         ║
║     keybinds could not be saved/loaded; added registration  ║
║     with keycode serialisation (Name string round-trip)     ║
║  7. Config keycode serialisation — SaveConfig / LoadConfig  ║
║     now encode KeyCode as its Name string; JSONEncode no    ║
║     longer errors on EnumItem values                        ║
║  8. Slider SetEnabled — added missing SetEnabled(v) method  ║
║     to match Toggle's API surface                           ║
║  9. Slider conns cleanup — AncestryChanged loop now uses    ║
║     ipairs (sequential table); pairs was technically wrong  ║
║ 10. AddSeparator / AddSection now return a Wrap API object  ║
║     so callers can call :SetVisible() / :Destroy() on them  ║
╚══════════════════════════════════════════════════════════════╝
--]]
...
local function tw(o, p, t, style, dir)
	...
	local tween = TweenService:Create(o,
...
-- ─────────────────────────────────────────────────────────────
--  DRAG  (mouse + touch) — v2.4
--
--  FIX 1 — CurrentCamera nil-guard:
--    doMove now checks workspace.CurrentCamera exists before
--    reading ViewportSize.  Avoids error on very early load.
--
--  FIX 2 — AbsoluteSize = 0 guard:
--    The drag connections are set up inside task.spawn with a
--    task.wait() so the UI has rendered at least one frame
--    and AbsoluteSize/AbsolutePosition are valid.
--
--  FIX 3 — Global listener idle early-exit:
--    c3 (UIS.InputChanged) returns immediately when not
--    dragging so it does zero work on every idle mouse-move.
-- ─────────────────────────────────────────────────────────────
local function MakeDraggable(win, handle)
	handle = handle or win

	local dragging  = false
	local dragStart = nil   -- Vector2: screen position at drag-begin
	...
	local function doMove(inputPos)
		...
		-- FIX 1: guard camera nil — can be nil on very early load
		local cam = workspace.CurrentCamera
		if not cam then return end
		local vp   = cam.ViewportSize
		-- FIX 2: skip if UI not yet rendered (AbsoluteSize still 0)
	...
	-- FIX 2: defer one frame so AbsoluteSize/Position are valid
	local c1, c2, c3, c4, c5
	task.spawn(function()
		task.wait()   -- wait one frame for layout to resolve
		...
		c2 = handle.InputChanged:Connect(function(input)
			if not dragging then return end   -- FIX 3: idle early-exit
			local t = input.UserInputType
		...
		-- FIX 3: global fallback — returns immediately when idle
		c3 = UserInputService.InputChanged:Connect(function(input)
			if not dragging then return end   -- FIX 3: zero cost when idle
			local t = input.UserInputType
			if t == Enum.UserInputType.MouseMovement or
			   t == Enum.UserInputType.Touch then
				doMove(input.Position)
	...
	-- Return object that disconnects ALL connections
	return {
		Disconnect = function()
...
function NexusUI:Notify(opt)
	...
	-- Dismiss
	local gone = false
	local entry = {}   -- Fix 5: reference stored in NotifList
	local function Dismiss()
		if gone then return end; gone = true
		-- Fix 5: remove from active list
...
function NexusUI:CreateWindow(opt)
	...
	local wSub    = opt.Subtitle or "v2.4"
