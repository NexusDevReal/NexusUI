--[[
╔══════════════════════════════════════════════════════════════╗
║  NexusUI  v2.4  —  Roblox Luau UI Library                    ║
╠══════════════════════════════════════════════════════════════╣
║  v2.4 — Stability & performance patch:                      ║
║                                                              ║
║  1. CurrentCamera nil-guard — drag doMove checks camera     ║
║     exists before reading ViewportSize; safe on early load  ║
║  2. AbsoluteSize=0 guard — drag setup deferred one frame    ║
║     via task.wait() so layout is resolved before first move ║
║  3. Global drag listener optimised — c3 (UIS.InputChanged)  ║
║     returns immediately when not dragging; no wasted work   ║
║     on every mouse-move frame while idle                    ║
║  4. UI parenting hardened — third fallback added:           ║
║     LocalPlayer.PlayerGui used if WaitForChild times out;   ║
║     ScreenGui will always be parented somewhere visible     ║
║  5. Notification cap — max 5 visible at once; oldest is     ║
║     dismissed automatically before a new one is added      ║
║  6. Tween cancellation — a TweenCache tracks the active     ║
║     tween per-object so it is :Cancel()'d before the next   ║
║     one plays; no more overlapping/fighting animations      ║
╚══════════════════════════════════════════════════════════════╝
--]]
