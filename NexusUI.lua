--[[
╔══════════════════════════════════════════════════════════════╗
║  NexusUI  v2.0  —  Roblox Luau UI Library                    ║
╠══════════════════════════════════════════════════════════════╣
║  v2.0 — Bug-fixed & enhanced build over v1.9:               ║
║                                                              ║
║  BUG FIXES                                                   ║
║  • Button: running flag now resets AFTER animation (was      ║
║    immediately, allowing callback spam)                      ║
║  • AddCredit: duplicate FadeLine calls removed               ║
║  • ProgressBar:Get() stores value directly (no text parse)   ║
║  • MakeDraggable: UIS.InputChanged now disconnected on       ║
║    window destroy — no more per-window connection leak       ║
║  • Dropdown/MultiDropdown open: task.wait() added in         ║
║    task.spawn so AbsolutePosition is always valid            ║
║  • WinAPI:Toggle() now sets Visible=false/true so the        ║
║    hidden window is not hit-testable                         ║
║  • TextBox NumberOnly: minus sign only allowed at pos 1      ║
║  • Keybind: ignores InputBegan when a TextBox has focus       ║
║                                                              ║
║  NEW FEATURES & IMPROVEMENTS                                 ║
║  • ▾/▲ chevrons (was "v")                                    ║
║  • ✓ checkmarks for selections (was ">")                     ║
║  • Window drag clamped to viewport bounds                    ║
║  • MultiDropdown gets search filter (parity with single)     ║
║  • Escape key closes any open dropdown                       ║
║  • WinAPI:SetTitle(t)  WinAPI:SetSubtitle(s)                 ║
║  • WinAPI:IsVisible() → bool                                 ║
║  • Slider: optional Step param for decimal precision         ║
║  • NexusUI:SetTheme(overrides) — patch any theme key         ║
║  • Notification: auto-creates fallback ScreenGui if called   ║
║    before any window (no longer silently returns)            ║
║  • Scrollbars: 4px (was 3px) — easier to grab               ║
║  • NexusUI.Version = "V2.0"                                   ║
╚══════════════════════════════════════════════════════════════╝
--]]
...
-- ─────────────────────────────────────────────────────────────
--  DRAG  (mouse + touch)
--  FIX: returns a cleanup function to disconnect the global
--       UIS.InputChanged listener — prevents per-window leak
-- ─────────────────────────────────────────────────────────────
local function MakeDraggable(win, handle)
	...
	-- FIX: store connection so caller can disconnect it
	local globalCon = UserInputService.InputChanged:Connect(function(i)
		if i == input and drag then
			local d = i.Position - mS
			-- IMPROVEMENT: clamp window to viewport so it can't be dragged off-screen
			local vp   = workspace.CurrentCamera.ViewportSize
			local winW = win.AbsoluteSize.X
			local winH = win.AbsoluteSize.Y
			local newX = math.clamp(fS.X.Offset + d.X, 0, vp.X - winW)
			local newY = math.clamp(fS.Y.Offset + d.Y, 0, vp.Y - winH)
			win.Position = UDim2.new(fS.X.Scale, newX, fS.Y.Scale, newY)
		end
	end)

	return globalCon  -- caller must :Disconnect() this on window destroy
end
...
function NexusUI:Notify(opt)
	...
	xb.Text = "✕"; xb.TextSize = 10; xb.Font = Enum.Font.GothamBold
...
function NexusUI:CreateWindow(opt)
	...
	local wSub    = opt.Subtitle or "v2.0"
	...
	Label(cBox,"✕",11,T.White,Enum.Font.GothamBold,
		Enum.TextXAlignment.Center,Enum.TextYAlignment.Center)
