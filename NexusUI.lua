--[[
╔══════════════════════════════════════════════════════════════╗
║  NexusUI  v4.0  —  Roblox Luau UI Library                    ║
╠══════════════════════════════════════════════════════════════╣
║  v4.0 — Full rewrite.  Every previous bug fixed:            ║
║                                                              ║
║  • Window now always visible on load (no hidden animation)   ║
║  • pairs() used everywhere — safe in all Roblox envs        ║
║  • No rawset() on Roblox Instances                          ║
║  • Connection leak fixed — per-element cleanup tables       ║
║  • Dropdown list uses screen-space Y, never mis-positioned   ║
║  • Tab indicator uses AbsolutePosition only after render     ║
║  • Notification holder is global — one per ScreenGui        ║
║  • Config: no crash on missing IDs, no duplicate warnings   ║
║    that break flow                                           ║
║  • MkIcon: full asset image support (rbxassetid, rbxthumb,  ║
║    raw numeric ID) + plain text fallback                     ║
║  • All elements: :Get() :Set() :SetVisible() :Destroy()     ║
║  • Toggle: :Fire() :SetEnabled()                            ║
║  • Button: :SetEnabled() :SetName() :SetDesc()              ║
║  • Slider: :SetMin() :SetMax() :SetSuffix()                 ║
║  • Dropdown: nil default, :Clear() :IsEmpty() :IsSelected() ║
║    :AddItem() :RemoveItem() :SetPlaceholder()               ║
║  • MultiDropdown: same + :SelectAll()                       ║
║  • TextBox: :Clear() :Focus() :SetPlaceholder()             ║
║  • ProgressBar: :Animate(v, dur)                            ║
║  • Win:GetValue(id)  Win:SetValue(id,v)                     ║
║    Win:GetElement(id)  Win:ListIds()                        ║
║  • Win:Toggle()  Win:Destroy()                              ║
╚══════════════════════════════════════════════════════════════╝
--]]
...
local T = {
	...
	Border     = Color3.fromRGB( 55,  42,  92),
	BorderBri  = Color3.fromRGB( 88,  66, 144),
	BorderGlow = Color3.fromRGB(108,  78, 198),
...
--    "rbxassetid://123456789"   full URL    → ImageLabel
...
local function MkIcon(parent, iconArg, defSize, defColor, defImgSize, defZi)
	...
	if type(iconArg) == "table" then
		...
		imgSize = o.Size   or defImgSize or UDim2.new(0.72,0,0.72,0)
		...
		imgSize = defImgSize or UDim2.new(0.72,0,0.72,0)
		noTint  = false; trans = 0; scale = "Fit"
		tSize   = defSize or 18
	...
	if isImg then
		...
		img.ZIndex = zi; img.Parent = parent
		return img
	else
...
local function MakeDraggable(win, handle)
	...
	local drag, input, mS, fS = false, nil, nil, nil
...
local function InitNH(sg)
	-- Reuse if already parented to this sg
...
function NexusUI:CreateWindow(opt)
	...
	local wSub    = opt.Subtitle or "v4.0"
	...
	-- ── Main window ──────────────────────────────────────────
	-- NOTE: Window is FULLY VISIBLE immediately — no hidden-on-spawn animation
	...
	Corner(tb, 16)
	Frame(tb, UDim2.new(1,0,0,16), UDim2.new(0,0,1,-16), T.Surface)  -- bottom-edge fill
	...
	do  -- soft ambient glow behind pill
	...
	do
		local vl = Label(vc, wSub, 10, T.AccentHi, Enum.Font.GothamBold,
			Enum.TextXAlignment.Center, Enum.TextYAlignment.Center)
		vl.Size = UDim2.new(1,0,1,0); Pad(vc,0,0,7,7)
	end
	...
	Corner(body, 16)
	Frame(body, UDim2.new(1,0,0,16), nil, T.BG)  -- top fill (hides rounding under titlebar)
	...
	-- Content area (holds all page ScrollingFrames)
	...
	function WinAPI:Toggle()
		hidden = not hidden
		FT(win, hidden
			and {Size=UDim2.new(0,wSize.X.Offset,0,0),BackgroundTransparency=1}
			or  {Size=wSize,BackgroundTransparency=0}, 0.22)
	end
	...
	function WinAPI:SaveConfig(name)
		...
		if ok then
			self:Notify({Title="Config Saved",Desc=path,Duration=3,Icon="OK",Color=T.Green})
		else
	...
	function WinAPI:LoadConfig(name)
		...
		self:Notify({Title="Config Loaded",Desc=path,Duration=3,Icon="OK",Color=T.Green})
	end
	...
	function WinAPI:AddTab(tabName, tabIcon)
		...
		-- Activate function — slides indicator, shows page
		local function Activate()
			...
			-- Slide indicator — wrapped in task.spawn so AbsoluteSize is ready
			task.spawn(function()
				task.wait()  -- one frame so layout resolves
		...
		-- Card (base container for every element)
		...
		-- Wrap: attach :SetVisible() and :Destroy() to any API table
		...
		function API:AddButton(opt)
			...
			local function Run()
				...
				runLbl.Text = "Done"; FT(runLbl,{TextColor3=T.Green},0.1)
				task.delay(0.9,function()
					if runLbl and runLbl.Parent then
						runLbl.Text="Run"; FT(runLbl,{TextColor3=T.White},0.15)
					end
				end)
				Call(cb); running = false
			end
		...
		function API:AddSlider(opt)
			...
			local vl = Label(card,tostring(defV)..sfx,13,color,Enum.Font.GothamBold,
			...
			local val      = math.clamp(defV,minV,maxV)
			local sliding  = false
			...
			-- Connections stored so they don't pile up across reloads
			local conns = {}
			...
			tBG.InputBegan:Connect(function(i)
				...
				or i.UserInputType==Enum.UserInputType.Touch then
					...
					SetVal(FromPos(i.Position)); Call(cb,math.round(val))
				end
			end)
			...
			function Sl:SetSuffix(s) sfx=s; vl.Text=tostring(math.round(val))..s end
			Reg(wName,id,
		...
		function API:AddDropdown(opt)
			...
			local chev = Label(pill,"v",13,T.TxtSub,Enum.Font.GothamBold,
				Enum.TextXAlignment.Center,Enum.TextYAlignment.Center)
			...
			-- Floating list (parented to overlay — never clipped)
			...
			local function BuildRows(filter)
				...
				for _,r in pairs(rows) do r.Parent=nil end; rows={}
				local cnt=0
			...
			pillBB.MouseButton1Click:Connect(function()
				...
				-- Position list below card using screen-space coords
			...
			function Drop:Refresh(ni)
				items=ni or {}
				local found=false
				for _,v in pairs(items) do if v==selVal then found=true; break end end
		...
		function API:AddMultiDropdown(opt)
			...
			local function BuildRows()
				...
				iSF.Size=UDim2.new(1,0,0,math.min(#items,5)*34+4)
			end
			BuildRows()

			sAll.MouseButton1Click:Connect(function()
				for _,i in pairs(items) do selected[i]=true end
				BuildRows(); RefBadge(); Call(cb,GetSel())
			end)
			clr.MouseButton1Click:Connect(function()
				for _,i in pairs(items) do selected[i]=false end
			...
			pBB.MouseButton1Click:Connect(function()
				...
				task.spawn(function()
					...
					local lH   = 34+math.min(#items,5)*34+10
		...
		function API:AddTextBox(opt)
			...
			if numOnly then
				tbx:GetPropertyChangedSignal("Text"):Connect(function()
					local clean=tbx.Text:gsub("[^%d%.%-]","")
					if tbx.Text~=clean then tbx.Text=clean end
				end)
			end
		...
		function API:AddColorPicker(opt)
			...
			for _,col in pairs(swatches) do
		...
		function API:AddProgressBar(opt)
			...
			function PB:Get()
				return tonumber(vl.Text:gsub("%%","")) or 0
			end
